...
router.post("/webhooks", async (req, res) => {
    ...
    if (payment.status === "succeeded") {
        ...
        let planInfoStatus = await updateFirebasePlanInfo(isAppendApplied, articleCount, stripePricing, customer);
...
const updateFirebasePlanInfo = async (isAppendApplied, articleCount, stripePricing, customer) => {
    try {
...
const cancelSubscription = async (subscriptions, oldPlanId) => {
    ...
    const subscription = await stripe.subscriptions.cancel(subscriptionIdToUpgrade);
    return 1;
};
