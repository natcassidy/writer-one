...
exports.markItemInError = exports.markItemInProgress = exports.markItemCompleted = exports.getNextItemFirebase = exports.addToQueue = exports.findFinetuneInFirebase = exports.addFinetunetoFirebaseUser = exports.decrementUserArticleCount = exports.getContextFromDb = exports.updateFirebaseJob = void 0;
exports.validateIpHasFreeArticle = validateIpHasFreeArticle;
exports.updateIpFreeArticle = updateIpFreeArticle;
exports.addArticleFieldToUserDocument = addArticleFieldToUserDocument;
...
exports.updateFirebaseJob = updateFirebaseJob;
...
exports.getContextFromDb = getContextFromDb;
...
exports.addFinetunetoFirebaseUser = addFinetunetoFirebaseUser;
...
exports.findFinetuneInFirebase = findFinetuneInFirebase;
...
exports.decrementUserArticleCount = decrementUserArticleCount;
const addToQueue = async (keyWord, internalUrls, tone, pointOfView, includeFAQs, currentUser, finetuneChosen, sectionCount, citeSources, isAmazonArticle, amazonUrl, affiliate, numberOfProducts, includeIntroduction = true, includeConclusion = false) => {
    try {
        await firebase_admin_1.default.firestore().collection("queue").add({
            keyWord,
            internalUrls,
            tone,
            pointOfView,
...
const getNextItemFirebase = async () => {
    ...
    const inProgressSnapshot = await firebase_admin_1.default
        .firestore()
        .collection("queue")
...
exports.getNextItemFirebase = getNextItemFirebase;
const markItemCompleted = async (itemId) => {
    try {
        // Update the item status to 'completed'
        await firebase_admin_1.default.firestore().collection("queue").doc(itemId).update({
            status: "completed",
        });
...
exports.markItemInError = markItemInError;
...
async function addArticleFieldToUserDocument(user) {
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            const docRef = firebase_admin_1.default.firestore().collection("customers").doc(user.uid);
            const doc = await docRef.get();
            if (doc.exists) {
                const data = doc.data();
                if ("articles" in data) {
                    console.log(`Articles field already exists for ${user.uid}. No action taken.`);
                    return;
                }
            ...
            console.error(`Attempt ${attempt + 1}: Error checking/adding article field to customer document for ${user.uid}:`, error);
        }
    }
    console.error(`Failed to check/add article field after ${maxRetries} attempts for ${user.uid}`);
}
